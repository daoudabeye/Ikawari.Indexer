...
namespace Blockcore.Indexer.Core.Sync.SyncTasks
{
   public class Notifier : TaskRunner<AddressNotifications>
   {
      ...
      /// </summary>
      public Notifier(IOptions<IndexerSettings> configuration, IOptions<ChainSettings> chainConfiguration, ILogger<Notifier> logger, IStorage storage, IConfiguration appConfig)
      ...
      {
            var notifyBatchCount = appConfig.GetOrDefault<int>("NotifyBatchCount", configuration.NotifyBatchCount);
         if (notifyBatchCount == 0)
         {
            Abort = true;
            return true;
         }
         ...
         if (TryDequeue(out AddressNotifications item))
         {
            ...
            do
            {
               var addresses = Extensions.Extensions.TakeAndRemove(queue, notifyBatchCount).ToList();
               ...
               try
               {
                  ...
                  await client.Value.PostAsync(appConfig.GetOrDefault<string>("NotifyUrl", configuration.NotifyUrl), coin, new JsonMediaTypeFormatter());
                  sendCount++;
               }
               catch (Exception ex)
               {
