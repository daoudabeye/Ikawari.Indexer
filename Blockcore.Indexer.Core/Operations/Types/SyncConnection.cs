...
namespace Blockcore.Indexer.Core.Operations.Types
{
   ...
   public class SyncConnection
   {
      ...
      /// </summary>
      public SyncConnection(IOptions<IndexerSettings> config, IOptions<ChainSettings> chainConfig, IOptions<NetworkSettings> networkConfig, IConfiguration appConfig)
      {
         ...
         Password = appConfig.GetOrDefault<string>("RpcPassword", configuration.RpcPassword);
         ...
         RpcAccessPort = appConfig.GetOrDefault<int>("RPCPort", configuration.RpcAccessPort);

         ApiAccessPort = appConfig.GetOrDefault<int>("APIPort", networkConfiguration.APIPort);

         var RpcDomain = configuration.RpcDomain;
         ServerDomain = appConfig.GetOrDefault<string>("RpcDomain", RpcDomain).Replace("{Symbol}", chainConfiguration.Symbol.ToLower());

         User = appConfig.GetOrDefault<string>("RpcUser", configuration.RpcUser);
         Secure = appConfig.GetOrDefault<bool>("RpcSecure", configuration.RpcSecure);
