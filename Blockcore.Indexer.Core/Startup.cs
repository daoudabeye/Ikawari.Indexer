      ...
      public static void AddIndexerServices(IServiceCollection services, IConfiguration configuration)
      {
         ...
         services.AddSingleton(_ =>
         {
            ...
            var chainConfiguration  = _.GetService(typeof(IOptions<ChainSettings>)) as IOptions<ChainSettings>;//  configuration.GetSection("Chain") as ChainSettings;
             ...
             var ConnectionString = configuration.GetOrDefault<string>("ConnectionString",
				 indexerConfiguration.Value.ConnectionString).Replace("{Symbol}", chainConfiguration.Value.Symbol.ToLower());

			var mongoClient = new MongoClient(ConnectionString);
             
            string dbName = configuration.GetOrDefault<bool>("DatabaseNameSubfix", indexerConfiguration.Value.DatabaseNameSubfix)
